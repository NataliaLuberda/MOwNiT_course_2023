...
def main(m):
    ...
    x = np.linspace(-math.pi, 2.0 * math.pi, 150, endpoint=True)#tu można zmienić liczbę węzłów na inną
    ...
    plt.title("Funkcja aproksymcji stopnia " + str(m) + " przy 150 węzłach")
    plt.xlabel('x')
    plt.ylabel('y')
...
if __name__ == '__main__':
    # Określamy stopnie m, dla których będziemy generować wykresy.
    k = [3,5,7,9,12]
    nearly = []
    error_max = []
    ...
    # zapisujemy DataFrame do pliku CSV z nazwą zawierającą wartość n = 150
    df.to_excel(f'new_data_aproks_150_n.xlsx')